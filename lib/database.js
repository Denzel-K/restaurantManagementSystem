...
const createDatabase = async () => {
  try {
    await db.promise().query(`CREATE DATABASE IF NOT EXISTS restaurant_management`);
...
const checkDatabase = async () => {
  try {
    const [databases] = await db.promise().query("SHOW DATABASES LIKE 'restaurant_management'");
...
const setupDatabase = async () => {
  try {
    ...
    // Re-establish the connection with the `restaurant_management` database
